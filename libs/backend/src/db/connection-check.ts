  ...
  try {
    ...
    if (error && typeof error === "object") {
      ...
      if (err.code === "ECONNREFUSED" || err.errno === "ECONNREFUSED") {
        console.error("👉 Diagnostic: Port unreachable. Check if PostgreSQL server is running and accessible on the specified host/port.");
      } else if (err.code === "28P01") {
        console.error("👉 Diagnostic: Invalid password or authentication failed. Check database credentials.");
      } else if (err.code === "3D000") {
        console.error("👉 Diagnostic: Database does not exist. Verify the database name in your connection string.");
      } else if (err.code === "ENOTFOUND") {
        console.error("👉 Diagnostic: Host name could not be resolved. Check internet/DNS settings or host configurations.");
      } else {
        console.error(`👉 Diagnostic: Error Code: ${err.code || "unknown"}. Message: ${err.message || String(error)}`);
      }
    } else {
