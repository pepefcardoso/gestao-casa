...
router.openapi(
  ...
  async (c): Promise<RouteConfigToTypedResponse<typeof postExpenseRoute>> => {
    try {
      ...
      if (!newExpense) {
        return c.json(badRequest("Failed to create expense"), 400);
      }
...
router.openapi(
  ...
  async (c): Promise<RouteConfigToTypedResponse<typeof putExpenseRoute>> => {
    try {
      ...
      if (!expense) {
        return c.json(notFound("Expense"), 404);
      }
      ...
      if (payload.roomId && payload.roomId !== expense.roomId) {
        ...
        if (room) {
          ...
          if (!roomCheck.success) {
            return c.json(
              forbidden("Cannot assign expense to a room you do not have write access to."),
              403,
            );
          }
